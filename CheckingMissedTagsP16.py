...
def p16_missed_species(i):
    ...
    p16_blast = pd.read_csv("221117-1910.P16N-S.16S.dna-sequences.tsv", sep='\t', header=None)
    ori_data = pd.read_csv("221118-0208.P16N-S.16S.all-16S-seqs.with-tax.proportions.tsv", sep='\t')
...
with mp.Pool(mp.cpu_count() - 1) as pool:
    data = pool.map(p16_missed_species, [i for i in range(p16_blast.shape[0])])
